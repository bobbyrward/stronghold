...
func (abis *AudiobookImporterSystem) ProcessImportType(ctx context.Context, importType config.ImportType, library *config.ImportLibrary) error {
	torrents, err := qbit.GetUnimportedTorrentsByCategory(
		...
		abis.qbitClient,
		importType.Category,
		abis.cfg.ImportedTag,
		abis.cfg.ManualInterventionTag,
	)
	if err != nil {
...
func (abis *AudiobookImporterSystem) importTorrent(ctx context.Context, importTorrent qbittorrent.Torrent, importType config.ImportType, library *config.ImportLibrary) {
	bookMetadata, err := abis.autoSelectMetadata(ctx, importTorrent, importType, library)
	if err != nil {
		slog.ErrorContext(ctx, "Failed to auto-select metadata for torrent",
			slog.String("name", importTorrent.Name),
			slog.String("hash", importTorrent.Hash),
			slogx.Error(err),
		)
...
func (abis *AudiobookImporterSystem) autoSelectMetadata(ctx context.Context, importTorrent qbittorrent.Torrent, importType config.ImportType, library *config.ImportLibrary) (metadata.BookMetadata, error) {
	...
	if ok {
		...
		if err == nil {
			slog.InfoContext(ctx, "book metadata found by ASIN",
				...
				slog.String("suimmary", bookMetadata.Summarize()),
			)
			return bookMetadata, err
		}

		slog.ErrorContext(ctx, "Failed to get audiobook files metadata, falling back to title lookup",
			slog.String("name", importTorrent.Name),
			slog.String("hash", importTorrent.Hash),
			slog.Any("files", files),
			slogx.Error(err),
	...
	slog.InfoContext(ctx, "Mapped torrent files to local paths", slog.Any("files", files), slog.Any("bookMetadata", bookMetadata))
